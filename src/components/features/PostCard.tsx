      ...
      <GlassCard className="overflow-hidden">
        ...
        <div className="p-6 md:p-8 space-y-4">
          <div className="flex items-center justify-between gap-4">
            ...
            <span className="text-xs font-mono text-foreground/50">{readTime}</span>
          </div>

          <div className="space-y-2">
            <h3 className="text-xl md:text-2xl font-black tracking-tight">{title}</h3>
            {excerpt ? <p className="text-foreground/60 line-clamp-3">{excerpt}</p> : null}
          </div>

          <div className="flex items-center gap-3 pt-2">
            <Avatar src={authorAvatarUrl ?? undefined} name={authorName} size={36} />
            <div>
